...
const TypewriterClient = ({ words, emoji, options }) => {
    ...
    return (
        <>
            <span style={{ height: '20px', display: "flex", alignItems: 'center', justifyContent: 'center', textAlign: 'center', gap: '5px' }}>
                {text}
                {isDelay && emoji && <span className="mb5" dangerouslySetInnerHTML={{ __html: `&#${emoji};` }} />}
            </span>
        </>
    )
}
